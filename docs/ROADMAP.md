...
- Stand 2026-07-16: Alle Systeme stehen und funktionieren mit Platzhaltern - Animation (Player + Enemy, alle 5 Zustände) und Dual-Grid-Terrain (isoliert bewiesen, dann in `Room`/`testmap.tmx` integriert). Es fehlt nur noch der echte Kunst-Tausch selbst (LibreSprite) - technische Spezifikation dafür (Frame-Layouts, Sprite-Sheet-Maße, Tileset-Bitmask-Zuordnung) in `docs/ART_SPEC.md`.
